...
private Vector!T readVec(T, PQType type)(ubyte[] arr, shared IConnection conn)
{
    ...
    while(arr.length > 0)
    {
        ...
        static if(__traits(compiles, pgator.db.pq.types.all.convert!type(arr)))
        {
            auto value = pgator.db.pq.types.all.convert!type(arr[0..length]); 
            builder.put(value);
            arr = arr[length..$];
        } else static if(__traits(compiles, pgator.db.pq.types.all.convert!type(arr, conn)))
        { 
            auto value = pgator.db.pq.types.all.convert!type(arr[0..length], conn); 
            builder.put(value); 
            arr = arr[length..$];
        } else
        {
